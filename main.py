...
class Main(QWidget, Ui_Main_Form):
    ...
    def createCrawlObj(self, scrawlUrl=""):
        urlScrapy = UrlScrapyManage(scrawlUrl=scrawlUrl)
    ...
    def startCrawl(self):
        nowUrl = self.lineEdit.text().strip()
        self.textEdit.setText("")
        self.clearTable()
        if nowUrl == "" or not self.urlRegex.match(nowUrl):
            self.writeLog("请输入正确格式的URL", color="red")
            return
        self.pushButton.setEnabled(False)
        self.pushButton_2.setEnabled(True)

        self.urlScrapy = self.createCrawlObj(nowUrl)
    ...
    def writeLog(self, log, color="black"):
        colorDic = {"black": "#000000", "red": "#FF0000"}
    ...
    def writeProgress(self, nowCount, remainCount):
        log = "正在访问第{0}个URL，还有{1}个URL需要访问".format(nowCount, remainCount)
